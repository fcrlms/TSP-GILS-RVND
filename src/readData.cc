...
void readData(int argc, char **argv, int *Dimension, double ***Mdist)
{
	...
	if (arquivo.compare("DIMENSION") == 0)
		in >> arquivo;
	...
	if (arquivo.compare("EDGE_WEIGHT_TYPE") == 0)
		in >> arquivo;
	...
	if (ewt == "EXPLICIT") {
		...
		string ewf;
		if (arquivo.compare("EDGE_WEIGHT_FORMAT") == 0)
			in >> arquivo;
		in >> ewf;
		...
		if (ewf == "FUNCTION") {
			...
			// Preencher Matriz Distancia
			for (int j = 1; j < N; j++) {
				for (int i = j + 1; i < N + 1; i++) { // fix here
					in >> dist[i][j];
					dist[j][i] = dist[i][j];
				}
			}
